...
func runMySQLExplainTasks(ctx context.Context, vcursor VCursor, tasks []mysqlExplainTask) (map[Primitive]map[string]json.RawMessage, error) {
	...
	if !ok {
		...
		log.Warning("VEXPLAIN MYSQLPLAN: VCursor does not implement MultiShardPerShardExecutor; returning plan without MySQL EXPLAIN output")
		return explainResults, nil
	}
	...
	for _, task := range tasks {
		...
		for i, res := range results {
			if res == nil || len(res.Rows) == 0 || len(res.Rows[0]) == 0 {
				...
				log.Warningf("VEXPLAIN MYSQLPLAN got an empty EXPLAIN result; omitting shard %s/%s from output",
