...
func (qp *QueryProjection) NeedsProjecting(
	...
	for _, se := range qp.SelectExprs {
		...
		rExpr := sqlparser.CloneExpr(expr)
		sqlparser.Rewrite(rExpr, func(cursor *sqlparser.Cursor) bool {
			...
			rewritten, tErr := semantics.RewriteDerivedExpression(col, tableInfo)
			if tErr != nil {
				err = tErr
				return false
			}
