...
func (kss *keyspaceState) ensureConsistentLocked() {
	...
	if kss.moveTablesState != nil && kss.moveTablesState.Typ != MoveTablesNone && kss.moveTablesState.State != MoveTablesSwitched {
		log.V(2).Infof("keyspace %s: MoveTables operation in progress, not marking consistent yet", kss.keyspace)
		return
	}
	...
	if primary == nil || len(primary.ShardTabletControls) > 0 {
		if primary == nil {
			log.V(2).Infof("keyspace %s: primary partition is nil, not marking consistent yet", kss.keyspace)
		} else {
			log.V(2).Infof("keyspace %s: ShardTabletControls still active (%d controls), not marking consistent yet",
	...
	for _, shard := range primary.ShardReferences {
		...
		if sstate == nil || !sstate.serving {
			log.V(2).Infof("keyspace %s, shard %s: shard state not ready (nil: %v, serving: %v), not marking consistent yet",
	...
	for shard, sstate := range kss.shards {
		if sstate.serving && !activeShardsInPartition[shard] {
			log.V(2).Infof("keyspace %s, shard %s: shard is serving in healthcheck but not in topology partition, not marking consistent yet",
				kss.keyspace, shard)
			return
		}
	}
