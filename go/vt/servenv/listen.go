...
// RegisterReusePortFlag registers the reuse-port flag.
func RegisterReusePortFlag(fs *pflag.FlagSet) {
	fs.BoolVar(&reusePort, "reuse-port", reusePort, "Enable SO_REUSEPORT when binding sockets; available on Linux 3.9+ (default false)")
}
