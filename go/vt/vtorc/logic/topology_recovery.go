...
func getCheckAndRecoverFunctionCode(analysisEntry *inst.ReplicationAnalysis) recoveryFunction {
	...
	case inst.DeadPrimary, inst.DeadPrimaryAndSomeReplicas:
		...
		if isInEmergencyOperationGracefulPeriod(analysisEntry.AnalyzedInstanceAlias) {
...
func executeCheckAndRecoverFunction(analysisEntry *inst.ReplicationAnalysis) (err error) {
	...
	if err != nil {
		logger.Errorf("executeCheckAndRecoverFunction: error on failure detection: %+v", err)
		return err
	}
