...
func (pool *ConnPool[C]) GetWithPriority(ctx context.Context, setting *Setting, priority float64, handle *WaitHandle[C]) (*Pooled[C], error) {
	if ctx.Err() != nil {
	...
	if setting == nil {
		return pool.get(ctx, priority, handle)
	}
	return pool.getWithSetting(ctx, setting, priority, handle)
}

// NewWaitHandle returns a WaitHandle to pass to a single GetWithPriority call on
// this pool.
func (pool *ConnPool[C]) NewWaitHandle() *WaitHandle[C] {
	return &WaitHandle[C]{wl: &pool.wait}
}
...
func (pool *ConnPool[C]) get(ctx context.Context, priority float64, handle *WaitHandle[C]) (*Pooled[C], error) {
	...
	if conn == nil {
		...
		conn, err = pool.wait.waitForConn(ctx, nil, *closeChan, uint(pool.config.maxWaiters.Load()), priority, handle, pool.config.waiterCapDryRun.Load())
		if err != nil {
...
func (pool *ConnPool[C]) getWithSetting(ctx context.Context, setting *Setting, priority float64, handle *WaitHandle[C]) (*Pooled[C], error) {
	...
	if conn == nil {
		...
		conn, err = pool.wait.waitForConn(ctx, setting, *closeChan, uint(pool.config.maxWaiters.Load()), priority, handle, pool.config.waiterCapDryRun.Load())
		if err != nil {
			if errors.Is(err, ErrPoolWaiterCapReached) || errors.Is(err, ErrPoolLoadShed) {
				return nil, err
			}
